...
El sílabo es el puente entre los Catálogos (taxonomía de cursos/temas/subtemas) y la Generación de Materiales PDF. Define qué temas y subtemas se evalúan en cada semana del ciclo académico, estableciendo la cantidad de preguntas (`question_count`) para cada subtema de acuerdo a lo configurado en la plantilla. La configuración específica de los tipos de materiales (Examen, Práctica, etc.) y los topes de preguntas a extraer se centralizan a nivel del Ciclo a través de **Plantillas de Material**. Esto simplifica el sílabo y evita retrabajos al generar. Sin un sílabo, el sistema no tiene distribución de contenido.
...
- Q: ¿Existe un límite máximo de preguntas que se pueden asignar a una misma semana en el sílabo? → A: Sí, el sistema advertirá si la suma total de preguntas asignadas en una semana excede el límite máximo de preguntas configurado en la Plantilla de Material para esa semana, pero técnicamente el límite a nivel base de datos es 100 preguntas por semana.
...
Como coordinador académico, quiero asignar a cada semana del sílabo los temas y subtemas que se evaluarán, junto con la cantidad de preguntas requeridas por cada subtema, para estructurar la matriz temática que alimentará a las Plantillas de Material con cantidades exactas.

**Independent Test**: Asignación del topic "Ecuaciones Lineales" con subtopic "Resolución Básica" y 5 preguntas a la semana 3 del sílabo. Verificación de que el registro de distribución se persiste correctamente. Edición de la cantidad de preguntas a 3 y verificación de la actualización.

**Acceptance Scenarios**:

1. **Given** un sílabo creado y vinculado a un ciclo, **When** el coordinador asigna un topic + subtopic a una semana activa, **Then** se crea un registro de `syllabus_distribution` con los IDs correspondientes asumiendo un `question_count` por defecto de `1`.
2. **Given** una distribución existente en una semana, **When** el coordinador modifica la cantidad de preguntas del subtema, **Then** el registro se actualiza con Optimistic UI (respuesta inmediata en la UI, sincronización en background).
...
1. **Given** un sílabo con distribuciones configuradas, **When** el coordinador accede a la vista resumen, **Then** se muestra una matriz de semanas × temas con las cantidades de preguntas asignadas y totales por fila/columna.
...
- **FR-002**: Cada registro de distribución (`syllabus_distribution`) MUST contener: `syllabus_id`, `week_number`, `topic_id`, `subtopic_id` y `question_count` (entero mayor a 0).
...
- **FR-007**: El sistema MUST validar que el `question_count` sea mayor a 0 y que la sumatoria por semana no exceda un límite técnico de 100.
...
- **syllabus_distribution**: Distribución por semana. Campos: `id` (uuid), `syllabus_id` (FK → syllabus), `week_number` (number), `topic_id` (FK → topic), `subtopic_id` (FK → subtopic, NOT NULL), `question_count` (integer, > 0, actúa como la cantidad de preguntas asignadas).
...
- **CHECK**: `question_count > 0` — la cantidad de preguntas debe ser un entero positivo.
