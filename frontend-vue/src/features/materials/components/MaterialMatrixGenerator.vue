...
<template>
  <Teleport to="body">
    ...
    <Transition enter-active-class="transform transition ease-in-out duration-300" enter-from-class="translate-x-full"
      ...
      <div v-if="isOpen"
        class="fixed inset-y-0 right-0 w-full max-w-4xl bg-white dark:bg-[#11111a] shadow-2xl z-50 flex flex-col border-l border-slate-200 dark:border-white/5">
        <div class="flex flex-col h-full bg-slate-50 dark:bg-[#11111a]">
                  ...
                  <div class="grid grid-cols-5 sm:grid-cols-8 lg:grid-cols-10 gap-2">
                    <button v-for="week in weeksList" :key="week" @click="selectIntersection(template.id, week)"
                      ...
                      <span v-if="weekStatusMap[`${template.id}_${week}`]"
                        class="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full" :class="{
          ...
          <transition enter-active-class="transition ease-out duration-300 transform"
            ...
            <div v-if="selectedTemplate && selectedWeek"
              class="bg-white dark:bg-[#1a1a24] border-t border-slate-200 dark:border-white/10 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] dark:shadow-[0_-10px_40px_rgba(0,0,0,0.5)] z-20 shrink-0">
              <div class="p-6">
                <div class="flex items-center gap-3 mb-5">
                  <div
                    class="w-10 h-10 rounded-full bg-indigo-100 dark:bg-indigo-500/20 flex items-center justify-center border border-indigo-200 dark:border-indigo-500/30">
                  ...
                  <div>
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">Revisión de preguntas y generación del documento final.</p>
                  </div>
                </div>
                ...
                <div class="flex flex-col sm:flex-row items-stretch gap-4">
                  ...
                  <div class="flex-1 flex flex-col gap-2">
                    <button @click="handleGenerate"
                      ...
                      <span class="text-[10px] opacity-80 mt-1 uppercase tracking-widest">{{
                        selectedTemplate.courses.length }}
                        Cursos a procesar</span>
                    </button>

                    <!-- Error Alert Box -->
                    <div v-if="generateError"
                      class="bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 text-rose-600 dark:text-rose-400 text-xs p-3 rounded-lg flex gap-2 items-start animate-pulse">
                ...
                <div v-if="showCoursesList && currentRequest"
                  class="mt-6 border-t border-slate-200 dark:border-white/5 pt-5 space-y-4">
                  <h4 class="text-xs font-black text-indigo-500 dark:text-indigo-400 uppercase tracking-widest">Listado
                    de Cursos
                    y Archivos</h4>
                  <div class="space-y-3">
                    <div v-for="course in currentRequest.courses" :key="course.courseId"
                      ...
                      <div class="flex items-center gap-3">
                        <!-- Status Badge -->
                        <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border" :class="{
    ...
    <Transition enter-active-class="transition-opacity duration-300" enter-from-class="opacity-0"
      enter-to-class="opacity-100" leave-active-class="transition-opacity duration-200" leave-from-class="opacity-100"
      leave-to-class="opacity-0">
      <div v-if="isReviewModalOpen" class="fixed inset-0 bg-slate-950/60 dark:bg-black/80 backdrop-blur-md z-[300]" />
    </Transition>

    <Transition enter-active-class="transform transition ease-out duration-300" enter-from-class="opacity-0 scale-95"
      enter-to-class="opacity-100 scale-100" leave-active-class="transform transition ease-in duration-200"
      ...
      <div v-if="isReviewModalOpen"
        ...
        <div
          class="bg-white dark:bg-[#1e1e2d] rounded-2xl shadow-2xl max-w-4xl w-full max-h-[85vh] overflow-hidden flex flex-col border border-slate-200 dark:border-white/5 pointer-events-auto relative">
          ...
          <button type="button"
            ...
            aria-label="Cerrar modal" @click="isReviewModalOpen = false">
