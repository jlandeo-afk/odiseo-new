...
const downloadPdf = async (course: any) => {
  ...
  try {
    ...
    toast.add({ title: 'Error', description: 'Ocurrió un error al descargar el PDF.', color: 'red', timeout: 3000 });
  }
};
...
const downloadMergedPdf = async () => {
  ...
  try {
    ...
    toast.add({ title: 'Error', description: 'Ocurrió un error al descargar el PDF combinado.', color: 'red', timeout: 3000 });
  }
};
          ...
          <div class="flex-1 flex flex-col lg:flex-row min-h-0">
            ...
            <transition enter-active-class="transition ease-out duration-300"
              ...
              <div v-if="selectedTemplate && selectedWeek"
                :key="`${selectedTemplate?.id}_${selectedWeek}`"
                      ...
                      <div class="space-y-2">
                        <div v-for="course in currentRequest.courses" :key="course.courseId + '_' + course.status"
                          ...
                          <div class="flex items-center gap-2 shrink-0">
                            ...
                            <UButton v-if="course.status === 'COMPLETED' || course.status === 'COMPLETED_WITH_WARNINGS'" size="xs" color="indigo"
