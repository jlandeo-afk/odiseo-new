...
export class QuestionSelectorService {
  ...
  async selectQuestions(job: GenerateMaterialJobDto): Promise<any[]> {
    ...
    const courseReq = await this.entityManager.findOne(MaterialRequestCourse, {
      where: { id: job.job_id },
    });

    if (courseReq) {
      // 2. Fetch all review questions for this parent request that belong to this course and are approved
      const reviewQuestions = await this.entityManager.createQueryBuilder(MaterialReviewQuestion, 'mrq')
